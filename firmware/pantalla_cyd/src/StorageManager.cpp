...
void StorageManager::initMac() {
    ...
    char clean[13];
    snprintf(clean, sizeof(clean), "%02X%02X%02X%02X%02X%02X",
             raw[0], raw[1], raw[2], raw[3], raw[4], raw[5]);
    _cleanMac = String(clean);
}
...
bool StorageManager::hasWifiConfig() {
    return _prefs.getString("w_ssid", "").length() > 0;
}
...
bool StorageManager::getTouchCalibration(uint16_t* calData) {
    if (!_prefs.isKey("cal_valid")) {
        return false;
    }
    size_t len = _prefs.getBytes("cal_data", calData, sizeof(uint16_t) * 5);
    return len == (sizeof(uint16_t) * 5);
}

void StorageManager::saveTouchCalibration(const uint16_t* calData) {
    _prefs.putBytes("cal_data", calData, sizeof(uint16_t) * 5);
    _prefs.putBool("cal_valid", true);
}
